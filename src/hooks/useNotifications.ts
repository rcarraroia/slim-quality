...
export function useNotifications(options: UseNotificationsOptions = {}) {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await apiService.get<NotificationsResponse>('/notifications?limit=5');
  ...
  const markAsReadMutation = useMutation({
    mutationFn: async (notificationId: string) => {
      const response = await apiService.put<{ message: string }>(`/notifications/${notificationId}/read`, {});
      return response.data;
    },
    onSuccess: () => {
  ...
  const markAllAsReadMutation = useMutation({
    mutationFn: async () => {
      const response = await apiService.put<{ message: string }>('/notifications/mark-all-read', {});
      return response.data;
    },
    onSuccess: () => {
      // Invalidar cache para atualizar lista e contador
...
export function useNotificationsPage(page: number = 1, limit: number = 20) {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await apiService.get<NotificationsResponse>(`/notifications?page=${page}&limit=${limit}`);
      return response.data;
    },
